 ...
 *
 * HTML Structure Tests for src/assignments/details.html
...
beforeAll(() => {
  const html = fs.readFileSync(
    path.resolve(__dirname, "../../src/assignments/details.html"), "utf8"
  );
...
test("[HTML-06] <h1 id='assignment-title'> exists", () => {
  expect(document.querySelector("h1#assignment-title")).not.toBeNull();
});
...
test("[HTML-08] <p id='assignment-due-date'> exists inside <main>", () => {
  expect(document.querySelector("main p#assignment-due-date")).not.toBeNull();
});

test("[HTML-09] <p id='assignment-description'> exists inside <main>", () => {
  expect(document.querySelector("main p#assignment-description")).not.toBeNull();
});

test("[HTML-10] <ul id='assignment-files-list'> exists inside <main>", () => {
  expect(document.querySelector("main ul#assignment-files-list")).not.toBeNull();
});
